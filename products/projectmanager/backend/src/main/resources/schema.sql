...
-- 자료 보존형 정형화 (plans/_opens/data_lineage_review) — 사라지거나 바뀐 것 한 건당 한 행
CREATE TABLE IF NOT EXISTS refine_trace (
  trace_id    BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  dataset_id  VARCHAR(80) NOT NULL,          -- 결과 데이터셋 (건너뛴 시트 = 업로드 배치)
  source_id   VARCHAR(300),                  -- 정제=원본 데이터셋 · 적재=파일/시트
  stage       VARCHAR(10) NOT NULL,          -- INGEST | REFINE | EDIT
  op          VARCHAR(40) NOT NULL,
  row_ref     INTEGER,                       -- 원본 행 번호 (적재=시트 행)
  col_name    VARCHAR(200),
  before_v    VARCHAR(4000),
  after_v     VARCHAR(4000),                 -- null = 삭제
  created_at  VARCHAR(20) NOT NULL
);
CREATE INDEX IF NOT EXISTS idx_trace_ds ON refine_trace (dataset_id, stage);

-- 사람 결정 — 다시 정제해도 유지된다. col_name '' = 모든 열 · row_ref -1 = 모든 행
CREATE TABLE IF NOT EXISTS refine_decision (
  source_id   VARCHAR(80) NOT NULL,
  op          VARCHAR(40) NOT NULL,
  col_name    VARCHAR(200) NOT NULL,
  row_ref     INTEGER NOT NULL,
  action      VARCHAR(10) NOT NULL,          -- KEEP = 그 정제를 하지 않음
  decided_at  VARCHAR(20) NOT NULL,
  PRIMARY KEY (source_id, op, col_name, row_ref)
);
