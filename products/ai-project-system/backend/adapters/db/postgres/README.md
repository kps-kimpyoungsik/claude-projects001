...
> **2026-07-26 아키텍처 정리 검토** — 이 디렉터리를 `_unwired/`/`experimental/` 등으로
> 이동하거나 삭제하는 방안을 검토했으나, 위 경고가 이미 미배선 상태를 명시하고 있어 이동은
> 이름표만 바꿀 뿐 실질 이득이 없다고 판단해 **그대로 유지하기로 결정**했다. 근거:
> `01_ARCHITECTURE_REFACTOR_PLAN.md` §2.
