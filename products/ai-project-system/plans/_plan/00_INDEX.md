...
> **별도 트랙(2026-07-25 추가, 설계만)**: `04_PHASE0_PROJECT_REGISTRATION.md` §8 — STEP 2
> (분야 선택) 대폭 보완. 사용자 지적("용어가 너무 어렵다") 실측 근본원인이 "설명 콘텐츠
> 부재"가 아니라 "이미 있는 설명(`codes.py`)을 프론트가 안 쓰고 있었다"는 배선 누락임을
> 규명 + HA(이중화) 조건부 아키텍처 옵션·그리드/LLM/리포트 정적 큐레이션 추천(WebSearch
> 조사, 2026-07-25 기준)·아키텍처/보안 지식페이지·보안레벨 선택·Mermaid 다이어그램
> 시각화·UI 개선(설명박스·타이틀)·MD 파일 정리 후보까지 포함. Stage5 작업분해(W1~W8),
> 구현은 다음 턴부터 W1(라벨/설명 배선)을 최우선으로 단계별 진행 권장.
